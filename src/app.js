...
const inputStyle = { width:"100%", background:C.bg, border:`1px solid ${C.border}`, borderRadius:7, padding:"9px 12px", color:C.text, fontSize:13, outline:"none", boxSizing:"border-box" };
const labelStyle = { fontSize:11, color:C.muted, textTransform:"uppercase", letterSpacing:0.5, display:"block", marginBottom:5 };
const filterBtn  = (active) => ({ background:active?C.pinkDim:C.surface, border:`1px solid ${active?C.pink:C.border}`, borderRadius:6, padding:"7px 12px", color:active?C.pink:C.muted, fontSize:12, cursor:"pointer", fontWeight:active?700:400 });
...
function ScreenerTab() {
  ...
  const [flow, setFlow] = useState(MOCK_FLOW);
  ...
  const getDTE = (exp) => !exp?999:Math.ceil((new Date(exp)-new Date())/86400000);
  ...
  const activeBtn = (active) => ({ background:active?C.pinkDim:C.surface, border:`1px solid ${active?C.pink:C.border}`, color:active?C.pink:C.muted, borderRadius:7, padding:"9px 14px", fontSize:12, cursor:"pointer", fontWeight:active?700:400 });
    ...
    <div>
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:11, padding:16, marginBottom:18, display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"flex-end", paddingTop:20 }}>
          ...
          <button onClick={()=>setAutoRefresh(a=>!a)} style={activeBtn(autoRefresh)}>{autoRefresh?"Auto ON":"Auto OFF"}</button>
        </div>
      ...
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:11, padding:16, marginBottom:18 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(148px,1fr))", gap:12 }}>
          {[
            ["Min Premium",[50000,100000,250000,500000,1000000,2000000],minPremium,v=>setMinPremium(Number(v)),fmtK],
            ["Min Vol/OI", [1,2,3,5,10,20],minVolOI,v=>setMinVolOI(Number(v)),v=>v+"x"],
            ["Min DTE",    [7,14,21,30,45],minDTE,v=>setMinDTE(Number(v)),v=>v+" days"],
            ["Max DTE",    [30,45,60,90,120,180],maxDTE,v=>setMaxDTE(Number(v)),v=>v+" days"],
          ].map(([label,opts,val,setter,fmtFn])=>(
            <div key={label}>
              ...
              <select value={val} onChange={e=>setter(e.target.value)} style={{ width:"100%", background:C.bg, border:`1px solid ${C.border}`, borderRadius:6, padding:"8px 10px", color:C.text, fontSize:13 }}>
                {opts.map(v=><option key={v} value={v}>{fmtFn(v)}</option>)}
              </select>
            </div>
          ))}
          <div>
          ...
          <div style={{ display:"flex", flexDirection:"column", justifyContent:"flex-end" }}>
            <button onClick={()=>setSweepOnly(s=>!s)} style={activeBtn(sweepOnly)}>{sweepOnly?"Sweeps Only":"All Orders"}</button>
          </div>
        </div>
      </div>
...
function JournalTab({ trades, setTrades }) {
  ...
  const submitClose = async () => {
    ...
    const { error } = await supabase.from("trades").upsert(record);
    if (!error) setTrades(p=>p.map(t=>t.id===closingId?record:t));
    setClosingId(null);
  };
      ...
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:11, overflow:"hidden" }}>
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
            <thead>
              <tr style={{ borderBottom:`1px solid ${C.border}` }}>
                {[["Ticker","ticker"],["Type","type"],["Strike","strike"],["Expiry","expiration"],["Bought","date_bought"],["Sold","date_sold"],["Paid","premium_paid"],["Sold For","premium_sold"],["Qty","contracts"],["P&L",null],["%",null],["Status",null],["",null]].map(([label,col],i)=>(
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100, padding:16 }}>
          <div style={{ background:C.surface, border:`1px solid ${C.borderHi}`, borderRadius:14, padding:26, width:"100%", maxWidth:500, maxHeight:"90vh", overflowY:"auto" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              {[["Ticker","ticker","AAPL"],["Strike","strike","150"],["Date Bought","date_bought","date"],["Expiration","expiration","date"],["Premium Paid","premium_paid","2.50"],["Contracts","contracts","1"],["Date Sold","date_sold","date"],["Premium Sold","premium_sold","Leave blank if open"]].map(([label,key,ph])=>(
                <div key={key}>
                  <label style={labelStyle}>{label}</label>
                  <input type={ph==="date"?"date":"text"} placeholder={ph!=="date"?ph:""} value={form[key]} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))} style={inputStyle} />
                </div>
              ))}
            </div>
            ...
            <div style={{ marginTop:12 }}>
              ...
              <textarea value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} placeholder="Signal source, thesis..."
...
function AnalyticsTab({ trades }) {
  ...
  const lineData=useMemo(()=>{
    const sorted=[...closed].sort((a,b)=>new Date(a.date_sold||0)-new Date(b.date_sold||0));
    let running=0;
      ...
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:"20px 16px", marginBottom:18 }}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={lineData} margin={{ top:4, right:16, bottom:4, left:8 }}>
            ...
            <YAxis tick={{ fill:C.muted, fontSize:10 }} axisLine={false} tickLine={false} tickFormatter={v=>fmtK(v)} />
        ...
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:"20px 16px" }}>
          ...
          <ResponsiveContainer width="100%" height={190}>
            <PieChart>
              ...
              <Legend iconType="circle" iconSize={9} formatter={v=><span style={{ color:C.textSub, fontSize:12 }}>{v}</span>} />
            </PieChart>
          </ResponsiveContainer>
...
// ─── ROUTER: /TBR2026 is the secret path ──────────────────────────
export default function App() {
  return (
    <BrowserRouter>
      <Routes>
        <Route path="/TBR2026" element={<BloomRoom />} />
        <Route path="*" element={<Navigate to="/TBR2026" replace />} />
      </Routes>
    </BrowserRouter>
  );
}
